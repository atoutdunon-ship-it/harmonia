...
- Code niveau **expert**
- **RÈGLE ABSOLUE — contraste texte** : tout fond foncé (navy, noir, dark) → texte en `#ffffff`. Toujours appliquer `color:#ffffff` en inline sur les éléments générés dynamiquement en JS. Ne JAMAIS utiliser `rgba(255,255,255,X)` avec X < 0.7 pour du texte sur fond sombre — utiliser `opacity` sur le conteneur si besoin d'atténuation. Rappelé et corrigé plusieurs fois — violation systématique interdite.
...
### Version actuelle
**v94** — vérifier avec `grep "v=" artistes.html | head -1`
...
*Mis à jour : JS v170 / CSS v161 — Juillet 2026*
